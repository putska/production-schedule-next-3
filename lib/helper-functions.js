...
const deconstructJobData = (jobs, categoryKey) => {
  ...
  newJobs = newJobs
    ...
    .map((job) => {
      ...
      newJobData.JSON = typeof job.JSON === "string" ? JSON.parse(job.JSON) : job.JSON;
      ...
      const keys = [
        "jobNumber", "jobName", "shopID", "wallType",
        "shopStart", "fieldStart",
        "engineering", "reserved", "booked",
        "weeks", "unitsPerWeek", "units", "weeksToGoBack",
        "metalTakeoff", "end"
      ];
...
const getDisplayUnits = (job, col, categoryKey) => {
...
const createCategoryData = (jobs, categoryKey, tabColumns) => {
  let newJobs = convertDates(jobs, categoryKey);
    ...
    tabColumns.forEach((col) => {
      if (col.columns) {
        col.columns.forEach((subCol) => {
          newJobData[subCol.dataField] = getDisplayUnits(job, subCol, categoryKey);
        })
      } else {
        newJobData[col.dataField] = getDisplayUnits(job, col, categoryKey);
      }
    })
    return newJobData;
  })
