...
const JobForm = (props) => {
  ...
  const [canEditShopStart, setCanEditShopStart] = useState(false);
  const [canEditWeeks, setCanEditWeeks] = useState(false);
  ...
  const overlapping = (newMetalTakeoff) => {
    let foundOverlappingDate = jobs.find((job) => {
      return (
        toMondayDate(job.metalTakeoff).toLocaleDateString() ===
        newMetalTakeoff.toLocaleDateString()
      );
    });
  ...
  const calculateMetalTakeoffDate = (startDate, weeksToGoBack) => {
    ...
    return {
      newMetalTakeoff: newMetalTakeoff,
      newWeeksToGoBack: newWeeksToGoBack,
    };
  };
    ...
    if (value != undefined) {
      ...
      if (newFormData.shopID && newFormData.unitsPerWeek == 0) {
        const shop = shopSettings.find(
          (shop) => shop.ID === newFormData.shopID
        );
        newFormData.unitsPerWeek =
          shop && shop.value === "La Verne" ? 125 : 150;
      }

      // weeks
      if (!newFormData.stickwall && newFormData.unitsPerWeek > 0) {
        newFormData.weeks = Math.ceil(
          newFormData.units / newFormData.unitsPerWeek
        );
      ...
      // shop start
      if (
        newFormData.shopStart == null &&
        newFormData.fieldStart != null &&
        newFormData.weeks > 0 &&
        !fieldStartFocused
      ) {
      ...
      if (
        newFormData.shopStart != null &&
        (option.dataField === "weeksToGoBack" ||
          newFormData.metalTakeoff == null)
      ) {
        const { newMetalTakeoff, newWeeksToGoBack } = calculateMetalTakeoffDate(
          newFormData.shopStart,
          newFormData.weeksToGoBack
        );
      ...
      // weeks to go back
      if (
        option.dataField === "shopStart" &&
        newFormData.shopStart != null &&
        newFormData.metalTakeoff != null
      ) {
        newFormData.weeksToGoBack = toWeeks(
          newFormData.shopStart,
          newFormData.metalTakeoff
        );
      }
        ...
        <Grid container spacing={2} direction="row">
          <Grid item xs={100}>
            <SelectBox
              ...
              onValueChanged={(e) =>
                handleInputChange({ dataField: "shopID" }, e.value)
              }
            />
          </Grid>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  ...
                  style={{ float: "left" }}
                  onChange={(e) =>
                    handleInputChange({ dataField: "booked" }, e.target.checked)
                  }
                />
              }
              label="Booked"
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "jobNumber" }, e.target.value)
              }
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "jobName" }, e.target.value)
              }
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "customer" }, e.target.value)
              }
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "wallType" }, e.target.value)
              }
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "unitsPerWeek" }, e.target.value)
              }
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "units" }, e.target.value)
              }
              disabled={formData.stickwall}
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              onChange={(e) =>
                handleInputChange({ dataField: "weeks" }, e.target.value)
              }
          ...
          <Grid item xs={6}>
            <DateBox
              ...
              onValueChanged={(e) =>
                handleInputChange(
                  { dataField: "fieldStart", editorType: "date" },
                  e.value
                )
              }
              type="date"
              ...
              onFocusOut={() => {
                ...
                if (
                  ...
                  const daysToAdd = newFormData.weeks * 7 * -1; // multiply by -1 bc start is before field start
                  newFormData.shopStart = addDays(
                    newFormData.fieldStart,
                    daysToAdd
                  );
                  newFormData.shopStart = toMondayDate(newFormData.shopStart);
                }
          ...
          <Grid item xs={6}>
            <DateBox
              ...
              onValueChanged={(e) =>
                handleInputChange(
                  { dataField: "shopStart", editorType: "date" },
                  e.value
                )
              }
              type="date"
          ...
          >
            <Alert severity="info">
              Weeks to go back automatically updated to avoid overlapping metal
              takeoff dates!
            </Alert>
          </Snackbar>
        </Grid>
