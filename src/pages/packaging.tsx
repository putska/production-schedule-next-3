...
export default function PackagingPage(props: any) {
    ...
    const [jobs, setJobs] = useState(deconstructJobData(loadedJobs))
    ...
    useEffect(() => {
        if (jobsData.length > 0) {
            const lastJob = jobsData[jobsData.length - 1];
            if (lastJob.shopStart && lastJob.weeks) {
                newEndDate = toMondayDate(addDays(lastJob.shopStart, lastJob.weeks * 7));
    ...
    useEffect(() => {
        ...
        newJobs.forEach((job: any, index: any) => {
            ...
            for (let key in job.JSON[categoryKey]) {
                ...
                const useShopStart = cellData.linkToShopStart && cellData.shopStartOffset > 0;
                const useFieldStart = cellData.linkToFieldStart && cellData.fieldStartOffset > 0;
                ...
                if (cellData.actual) {
                    if (useFieldStart) {
                        ...
                        // assign display units to appropriate innerCol value
                        const calculatedDatafield = shopStartOffset + displayUnits.shopStartOffset;
                        newJobs[index][calculatedDatafield] = displayUnits;
                    }
                }
            }
